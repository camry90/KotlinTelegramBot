...
class LearnWordsTrainerTest {
    ...
    fun `test statistics with 4 words of 7`() {
        val trainer = LearnWordsTrainer(FileUserDictionary("src/test/4_words_of_7.txt"))
    ...
    fun `test statistics with corrupted file`() {
        val trainer = LearnWordsTrainer(FileUserDictionary("src/test/corrupted_file.txt"))
        kotlin.test.assertEquals(
    ...
    fun `test getNextQuestion() with 5 unlearned words`() {
        val trainer = LearnWordsTrainer(FileUserDictionary("src/test/5_unlearned_words.txt"))
    ...
    fun `test getNextQuestion() with 1 unlearned words`() {
        val trainer = LearnWordsTrainer(FileUserDictionary("src/test/1_unlearned_word.txt"))
        val question = trainer.getNextQuestion()
        assertTrue(question?.variants?.size == 4)
        assertTrue(question?.variants?.contains(question.correctAnswer) == true)
    }
    ...
    fun `test getNextQuestion() with all words learned`() {
        val trainer = LearnWordsTrainer(FileUserDictionary("src/test/all_words_learned.txt"))
        val question = trainer.getNextQuestion()
        assertNull(question)
    }
    ...
    fun `test checkAnswer() with true`() {
        val trainer = LearnWordsTrainer(FileUserDictionary("src/test/check_answer.txt"))
    ...
    fun `test resetProgress() with 2 words in dictionary`() {
        val trainer = LearnWordsTrainer(FileUserDictionary("src/test/reset_progress.txt"))
